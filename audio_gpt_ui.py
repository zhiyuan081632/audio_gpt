...
def execute_task(input_entry, execute_button, status_label):
    input_arg = input_entry.get()
    
    # 禁用执行按钮，防止多次点击
    execute_button.config(state=tk.DISABLED)
    
    # 更新状态标签为"运行中"
    status_label.config(text="转录中...")
    
    # 创建一个新线程来执行任务
    task_thread = threading.Thread(target=perform_task, args=(input_arg, execute_button, status_label))
    task_thread.start()

def perform_task(input_arg, execute_button, status_label):
    try:
        # 执行原有任务的代码，将input_arg传递给相应的函数
        audio_gpt(input_arg)
        
        # 任务完成后启用执行按钮
        execute_button.config(state=tk.NORMAL)
        
        # 更新状态标签为"转录完成"，并显示提示信息
        status_label.config(text="转录完成")
        tk.messagebox.showinfo("提示", "转录结果保存在./202xxxxx/srt")
    except Exception as e:
        # 如果出现错误，更新状态标签并显示错误信息
        status_label.config(text="出现错误")
        tk.messagebox.showerror("错误", str(e))

# 创建主窗口
root = tk.Tk()
root.title("B站/本地音频转录工具")

# 创建标签和文本框用于输入URL或选择目录
input_label = tk.Label(root, text="输入URL或选择目录:")
input_label.pack()

input_entry = tk.Entry(root, width=40)
input_entry.pack()

browse_button = tk.Button(root, text="浏览", command=partial(browse_directory, input_entry))
browse_button.pack()

# 创建执行按钮
execute_button = tk.Button(root, text="转录", command=lambda: execute_task(input_entry, execute_button, status_label))
execute_button.pack()
